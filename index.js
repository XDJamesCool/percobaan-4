/*
]=====> XDJames <=====[ ]=====> 0282297358232 <=====[
*/
// ANAK ANJING PASTI YANG ATAS DIUBAH
// NGOTAK KONSOL
// NUMPANG NAMA TAK MEMBUAT MU PRO
const {
   WAConnection,
   MessageType,
   Presence,
...
const { wait, simih, getBuffer, h2k, generateMessage, getGroupAdmins, getRandom, banner, start, info, success, close } = require('./lib/functions')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:XDJanes\n' // GANTI NAMA LU
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;wa="6282297358232:+"62 822-9735-8232\n' // GANTI NOMOR LU
            + 'END:VCARD'
// UDAH SEGITU KONSOL KEBAWAH BIARIN AJA
const ngonsol = JSON.parse(fs.readFileSync('./settings/XDJames.json'))
const {
    botName,
    ownerName,
    BarBarKey,
...
const getLevelingXp = (sender) => {
        ...
        const addLeveling = (sender) => {
            const obj = {: sender, xp: 1, level: 1}
            _level.push(obj)
            fs.writeFileSync('./database/user/level.json', JSON.stringify(_level))
        }
             
         const getRegisteredRandom = () => {
            return _registered[Math.floor(Math.random() * _registered.length)].
        }

        const addRegisteredUser = (user, sender, age, time, serials) => {
            const obj = { : user, name: sender, age: age, time: time, serial: serials }
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color('POWERED BY XDJames'))
})

	baby.on('credentials-updated', () => {
		fs.writeFileSync('./XDJames.json', JSON.stringify(baby.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'ingfokan cuyy...')
	})
	fs.existsSync('./XDJames.json') && baby.loadAuthInfo('./XDJames.json')
	baby.on('connecting', () => {
		start('2', 'XDJames Connecting...')
	})
	baby.on('open', () => {
		success('2', 'XDJames Connected')
	})
	baby.connect({timeoutMs: 30*1000})

baby.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.j)) return
		try {
			const mdata = await baby.groupMetadata(anu.j)
			console.log(anu)
			if (anu.action == 'add') {
				...
				try {
					...
					ppimg = 'https://i0.wp.com/www.gambarunik./wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `[ *WELCOME IN GC ${mdata.subject}* ] \n___________________________\n@${num.split('@')[0]} Intro/Dikick!!! \n➸ Nama : \n➸ Umur : \n➸ Askot : \n➸ Gender : \n➸ Udah Punya Doi/Blm: \n➸ Pap Muka dumlu!!! \n➸ Instagram? \n𝐒𝐚𝐯𝐞 𝐍𝐨𝐦𝐨𝐫 𝐀𝐃𝐌𝐈𝐍! \n *___________________________*\nJangan jadi kutu lomcat sayang!!`
				let buff = await getBuffer(ppimg)
				baby.sendMessage(mdata., buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJ": [num]}})
	...
	baby.on('message-new', async (mek) => {
		try {
			...
			if (mek.key && mek.key.remoteJ == 'status@broadcast') return
			if (mek.key.fromMe) return
			global.prefix
			global.blocked
			const content = JSON.stringify(mek.message)
			const from = mek.key.remoteJ
			const type = Object.keys(mek.message)[0]
			const { text, extendedText, contact, location, liveLocation, image, veo, sticker, document, audio, product } = MessageType
			const time = moment.tz('Asia/Jakarta').format('DD/MM HH:mm:ss')
			const timi = moment.tz('Asia/Jakarta').add(30, 'days').calendar();
			const timu = moment.tz('Asia/Jakarta').add(20, 'days').calendar();
            body = (type === 'conversation' && mek.message.conversation.startsWith(prefix)) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption.startsWith(prefix) ? mek.message.imageMessage.caption : (type == 'veoMessage') && mek.message.veoMessage.caption.startsWith(prefix) ? mek.message.veoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text.startsWith(prefix) ? mek.message.extendedTextMessage.text : ''
			budy = (type === 'conversation') ? mek.message.conversation : (type === 'extendedTextMessage') ? mek.message.extendedTextMessage.text : ''
            var tas = (type === 'conversation' && mek.message.conversation) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption ? mek.message.imageMessage.caption : (type == 'veoMessage') && mek.message.veoMessage.caption ? mek.message.veoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text ? mek.message.extendedTextMessage.text : ''
			...
			const q = args.join(' ')
			const botNumber = baby.user.j
			const sender = isGroup ? mek.participant : mek.key.remoteJ
			...
			const group = isGroup ? groupMetadata.j : ''
...
*/
			...
			const mentions = (teks, memberr, ) => {
				( == null ||  == undefined ||  == false) ? baby.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJ": memberr}}) : baby.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJ": memberr}})
			}
			const sendImage = (teks) => {
		    baby.sendMessage(from, teks, image, {quoted:mek})
		    }
		    const costum = (pesan, tipe, target, target2) => {
			baby.sendMessage(from, pesan, tipe, {quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJ: from } : {}) }, message: { conversation: `${target2}` }}})
			}
		    const sendPtt = (teks) => {
...
*/
            ...
            const check = getLeveling(sender)
            try {
                if (currentLevel === undefined && check === undefined) addLeveling(sender)
...
// ANTI LINK GRUP
			...
			const isMedia = (type === 'imageMessage' || type === 'veoMessage')
			const isQuotedImage = type === 'extendedTextMessage' && content.includes('imageMessage')
			const isQuotedVeo = type === 'extendedTextMessage' && content.includes('veoMessage')
...
*/
				...
				if (isBanned) return reply(nad.baned())
					...
					if ((isMedia && !mek.message.veoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.toFormat('webp')
							.save(ran)
					} else if ((isMedia && mek.message.veoMessage.seconds < 11 || isQuotedVeo && mek.message.extendedTextMessage.contextInfo.quotedMessage.veoMessage.seconds < 11) && args.length == 0) {
						const encmedia = isQuotedVeo ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await baby.downloadAndSaveMediaMessage(encmedia)
						ran = getRandom('.webp')
						reply(nad.wait())
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								console.log(`Error : ${err}`)
								fs.unlinkSync(media)
								tipe = media.endsWith('.mp4') ? 'veo' : 'gif'
								reply(nad.stikga())
							})
							.on('end', function () {
								console.log('Finish')
						...
						reply(`Kirim gambar/veo/gif dengan caption \n${prefix}sticker (durasi sticker veo 1-9 detik)`)
					}
					break
				case 'nulis':
                  if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis XDJames baik hati`)
				nul = body.slice(7)
				...
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis1 XDJames baik hati`)
				XDJames = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				buff = await getBuffer(`https://api.xteam.xyz/magernulis2?text=${XDJames}&APIKEY=${XteamKey}`)
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis2 XDJames baik hati`)
				laysha = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}ninjalogo XDJames & Gans`)
				reply(nad.wait())
		...
		case 'halloweentext':
		    ...
		    break
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub XDJames & Hub`)
				reply(nad.wait())
		...
		case 'textlight':
		    ...
		    break
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}glitchtext XDJames & Gans`)
					reply(nad.wait())
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return baby.sendMessage(from, 'Kode bahasanya mana kak? contoh : ${prefix}tts  Halo XDJames', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return baby.sendMessage(from, `Teksnya mana kak | contoh : ${prefix}tts  ah yamate kudasai`, text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata XDJames`
...
// PREMIUM
				case 'premiumlist':
					...
					baby.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJ": premium}})
					break
				case 'bokep':
				baby.updatePresence(from, Presence.composing) 
                  if (isBanned) return reply(nad.baned())
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
                if (isGroup) return  reply( 'TAK BISA DI GRUP KAK')
                anug = getRegisteredRandom(_registered).replace('@s.whatsapp.net','')
		 ...
		 case 'tomp3':
				...
				if (!isPrem) return reply(nad.premium())
                	baby.updatePresence(from, Presence.composing) 
					if (!isQuotedVeo) return reply('*Reply veo nya lah-_-*')
					reply(nad.wait())
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await baby.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
				...
				if (!isPrem) return reply(nad.premium())
					...
					media = await baby.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${media} -af equalizer=f=94:wth_type=o:wth=2:g=30 ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(media)
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						baby.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
				...
				case 'yutubdl':
					...
					baby.sendMessage(from, buff, veo, {mimetype: 'veo/mp4', quoted: mek})
					await limitAdd(sender)
					break
				case 'tiktod':
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply('URL NYA TAK VAL KAK')
					...
					baby.sendMessage(from, rmln, veo, {mimetype: 'veo/mp4', quoted: mek})
					await limitAdd(sender)
					break
					case 'hetag5':
                  if (isBanned) return reply(nad.baned())
				...
				if (!isPrem) return reply(nad.premium())
					...
					mem.push(adm..replace('c.us', 's.whatsapp.net'))
					})
					var options = {
					text: value,
					contextInfo: { mentionedJ: mem },
					quoted: mek
					}
...
*/
           ...
           if (isLimit(sender)) return reply(nad.limitend(pusname))
					apakah = body.slice(1)
					const apa =['Iya','Tak','Bisa Jadi','Ulangi bro gak paham']
...
case 'neontext':
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}neontext XDJames`)
					neon = body.slice(10)
					reply('「❗」WAIT GANS')
		   ...
		   await limitAdd(sender)
                ...
                if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					const trut =['Pernah suka sama siapa aja? berapa lama?','Kalau boleh atau kalau mau, di gc/luar gc siapa yang akan kamu jadikan sahabat?(boleh beda/sma jenis)','apa ketakutan terbesar kamu?','pernah suka sama orang dan merasa orang itu suka sama kamu juga?','Siapa nama mantan pacar teman mu yang pernah kamu sukai diam diam?','pernah gak nyuri uang nyokap atau bokap? Alesanya?','hal yang bikin seneng pas lu lagi sedih apa','pernah cinta bertepuk sebelah tangan? kalo pernah sama siapa? rasanya gimana brou?','pernah jadi selingkuhan orang?','hal yang paling ditakutin','siapa orang yang paling berpengaruh kepada kehupanmu','hal membanggakan apa yang kamu dapatkan di tahun ini','siapa orang yang bisa membuatmu sange','siapa orang yang pernah buatmu sange','(bgi yg muslim) pernah ga solat seharian?','Siapa yang paling mendekati tipe pasangan ealmu di sini','suka mabar(main bareng)sama siapa?','pernah nolak orang? alasannya kenapa?','Sebutkan kejadian yang bikin kamu sakit hati yang masih di inget','pencapaian yang udah dapet apa aja ditahun ini?','kebiasaan terburuk lo pas di sekolah apa?']
...
*/		 
				...
				if (!isRegistered) return reply(nad.noregis())
					if (isLimit(sender)) return reply(nad.limitend(pusname))
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}blackpink XDJames`)
					pink = body.slice(11)
				...
				if (!isRegistered) return reply(nad.noregis())	     	
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJ
           ...
           case 'demote':
				...
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJ
					...
					var bot = gh.split("&")[2];
					baby.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJ: from } : {}) }, message: { conversation: `${target}` }}})
					await limitAdd(sender)
					break
                      case 'leave':
                      if (isBanned) return reply(nad.baned())      
...
*/
                ...
                if (checkATMuser(sender) < jumblah) return reply(`uang mu tak mencukupi untuk melakukan transfer`)
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tak di aktifkan sama owner XDJames`)
                      if (isOwner) {
                      const one = 999999999
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
			data = await fetchJson(`https://tobz-api.herokuapp.com/api/moddro?q=${body.slice(10)}&apikey=BotWeA`)
			hepi = data.result[0] 
			...
			break
				...
				case 'info':
					...
					teks = `*➸ Nama bot* : ${me.name}\n*➸ OWNER* : ${ownerName}\n*➸ AUTHOR* : 𝗥𝗮𝗺𝗹𝗮𝗻 𝗜𝗗\n*➸ Nomor Bot* : @${me.j.split('@')[0]}\n*➸ Prefix* : ${prefix}\n*➸ Total Block Contact* : ${blocked.length}\n*➸ The bot is active on* : ${kyun(uptime)}`
					buffer = await getBuffer(me.imgUrl)
					baby.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJ: [me.j]}})
					break
            case 'admin':
            case 'owner':
            case 'creator':
...
if (!isRegistered) return reply(nad.noregis())
if (isLimit(sender)) return reply(nad.limitend(pusname))
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}hartatahta XDJames`)
har = body.slice(12)
...
case 'cloudtext':
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}cloudtext XDJames`)
cloud = body.slice(11)
...
*/
				...
				case 'bc':
					...
					if (isMedia && !mek.message.veoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await baby.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							baby.sendMessage(_.j, buff, image, {caption: `*「 BABY BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.j, `*「 BABY BROADCAST 」*\n\n${body.slice(4)}`)
						}
						reply('*「 SUKSES BOSKU 」*')
					}
					break
				case 'bcgc':
					...
					if (isMedia && !mek.message.veoMessage || isQuotedImage) {
						...
						buff = await baby.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							baby.sendMessage(_.j, buff, image, {caption: `*「 BC GROUP 」*\n\n➸ Dari Grup : ${groupName}\n➸ Pengirim : wa.me/${(sender.split('@')[0])}\n➸ Pesan : ${body.slice(6)}`})
						}
						reply('*「 SUKSES BOSKU 」*')
					} else {
						for (let _ of anu) {
							sendMess(_.j, `*「 BC GROUP 」*\n\n➸ Dari Grup : ${groupName}\n➸ Pengirim : wa.me/${(sender.split('@')[0])}\n➸ Pesan : ${body.slice(6)}`)
						}
						reply('*「 SUKSES BOSKU 」*')
					}
					break
				...
				case 'clone':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJ[0]
					let { j, , notify } = groupMembers.find(x => x.j === mentioned)
					try {
						pp = await baby.getProfilePicture()
						buffer = await getBuffer(pp)
						baby.updateProfilePicture(botNumber, buffer)
						mentions(`Foto profile Berhasil di perbarui menggunakan foto profile @${.split('@')[0]}`, [j], true)
					} catch (e) {
						reply(nad.stikga())
					}
					await limitAdd(sender)
